const Footer = ()=> {
    return (
        <div className="w-full ">
                ...
                <div className="basis-1/2 flex gap-4">
                    ...
                    <div className="basis-1/3">
                        ...
                        {["home", "work", "careers",'contact'].map((item, index) => (
                            <a key={index} className="block mt-2 capitalize text-zinc-200" href="#">{item}</a>
                        ))}
                    </div>
            ...
            <div className='max-w-screen-sm ml-8  gap-10 p-2 text-sm   flex '>
                {["Privacy Policy", "Cookie Policy", "Impressum",'Terms'].map((item, index) => (
                    <a key={index} className="block mt-2 capitalize text-zinc-400" href="#">{item}</a>
                ))}
            </div>
        </div>
    );
